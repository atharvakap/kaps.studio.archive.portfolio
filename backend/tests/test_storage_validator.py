...
async def test_validator_success():
    ...
    upload_file = UploadFile(
        filename="test.png", file=fake_file, headers={"content-type": "image/png"}
    )
...
async def test_validator_invalid_type():
    ...
    upload_file = UploadFile(
        filename="virus.exe", file=fake_file, headers={"content-type": "application/x-msdownload"}
    )
...
async def test_validator_too_large():
    ...
    fake_file = io.BytesIO(b"0123456789")
    upload_file = UploadFile(
        filename="large.png", file=fake_file, headers={"content-type": "image/png"}
    )
