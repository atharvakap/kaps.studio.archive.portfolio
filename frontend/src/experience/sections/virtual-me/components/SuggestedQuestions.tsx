...
export const SuggestedQuestions = ({ onSelect }: SuggestedQuestionsProps) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 px-4 pointer-events-auto">
      {suggestions.map((item, index) => (
        <button
          key={index}
          onClick={() => onSelect(item.text)}
          className="flex items-start gap-3 p-4 text-left bg-white/40 hover:bg-white/60 backdrop-blur-md border border-white/40 hover:border-white/80 rounded-2xl transition-all duration-300 group shadow-sm hover:shadow-md"
