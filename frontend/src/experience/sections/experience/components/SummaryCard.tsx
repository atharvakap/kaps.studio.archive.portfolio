...
export const SummaryCard = ({ experiences, isLoading }: SummaryCardProps) => {
  ...
  return (
    <div
      className="h-auto lg:h-full w-full flex flex-row lg:flex-col items-center justify-between lg:justify-center gap-4 glass-card rounded-3xl lg:rounded-4xl p-4 sm:p-5 lg:p-4 text-center relative overflow-hidden group"
      role="region"
      aria-label="Career Statistics"
    >
      <div className="w-10 h-10 lg:w-16 lg:h-16 rounded-full bg-white dark:bg-white/5 shadow-sm flex items-center justify-center lg:mb-6 group-hover:scale-105 transition-transform duration-500 shrink-0">
        <Briefcase
          size={22}
          strokeWidth={1.5}
      ...
      <h3 className="hidden lg:block text-xl lg:text-2xl font-semibold tracking-tight mb-2 lg:mb-1 text-black dark:text-black opacity-70">
        My Journey
      </h3>
      <p className="hidden lg:block text-xs lg:text-xs text-(--text) opacity-60 leading-relaxed mb-6 lg:mb-1">
      ...
      <div className="flex w-auto lg:w-full justify-center items-center gap-4 sm:gap-6 lg:mt-auto shrink-0">
        <div className="flex flex-col items-center">
          <span className="text-xl sm:text-2xl lg:text-3xl font-semibold text-[#FF6B00]">
